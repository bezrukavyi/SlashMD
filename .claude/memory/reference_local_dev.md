...
description: How to build, run, and test the Markeasy extension locally during development
type: reference
---------------

## Building and Testing Markeasy Locally
...
**2. Press F5** in VS Code and select **"Run Markeasy Extension"**.

Open any `.md` file in the new window. On save, reload with **Ctrl+Shift+P → Developer: Reload Webview**.

### Option B — Clean build + F5

Press **F5** and select **"Run Markeasy Extension (with build)"** — runs `npm run build` first via pre-launch task.
...
The output is at **`packages/extension-host/markeasy.vsix`** (not the project root).

Install it with:

```bash
code --install-extension packages/extension-host/markeasy.vsix
```

Or in VS Code: **Extensions → ⋯ → Install from VSIX…** and navigate to `packages/extension-host/markeasy.vsix`.
...
- Extension host logs: **Output panel → Markeasy channel**.
