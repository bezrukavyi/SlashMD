...
description: How to develop, iterate, debug, and test changes in Markeasy — local workflow, reload strategies, debugging tools, and testing approaches
type: reference
---

# Markeasy — Development Guide
...
Open the **monorepo root** (`Markeasy/`) as your VS Code workspace — the launch configurations in `.vscode/launch.json` and the tasks in `.vscode/tasks.json` only work when the root is the workspace.
...
- Select **"Run Markeasy Extension"** from the dropdown at the top
...
In the Extension Development Host window, open any `.md` file (even an empty one). The Markeasy editor activates automatically and shows your latest built code.
...
- **Console** — all `console.log` from `webview-ui/` appear here; messages are prefixed with `Markeasy:`
...
```
Output panel → "Extension Host" (or search for "Markeasy")
```

### Tracing the message protocol

Every message crossing the UI↔Host boundary is logged at both ends:
- **Host side** (Output panel): `Markeasy: Received validated message from webview: APPLY_TEXT_EDITS`
- **Webview side** (DevTools console): `Markeasy App: Handling message: DOC_INIT`
...
3. Switch to the raw text editor (`Ctrl+Shift+P → Markeasy: Open as Raw Markdown`) and inspect the output
4. Switch back to Markeasy to verify the editor re-parses it correctly
...
2. Open VS Code settings (`Ctrl+,`) and search `markeasy` to confirm the new setting appears
...
5. Switch back to Markeasy — confirm it re-parses correctly (round-trip)
...
- [ ] Switch back to Markeasy — confirm it re-parses without loss
...
**External changes**
- [ ] Edit the raw `.md` file in a text editor while Markeasy is open — confirm the editor updates

**Settings**
- [ ] Change `markeasy.theme.fontScale` → confirm font size updates without reload
- [ ] Change `markeasy.theme.codeTheme` → confirm syntax colours update
- [ ] Change `markeasy.theme.h1Color` → confirm heading colour updates
...
npm run package        # produces packages/extension-host/markeasy.vsix
code --install-extension packages/extension-host/markeasy.vsix --force
```
