...
description: Full guide to the Markeasy codebase structure, data flow, and main development workflows
type: reference
---------------

# Markeasy — Project Structure & Workflows
...
| `src/customEditor.ts` | `MarkeasyEditorProvider` — webview lifecycle, message routing, document sync |
| `src/types.ts`        | `MarkeasySettings` interface, `getSettings()`, `getThemeOverrides()`         |
| `src/assetService.ts` | Handles image paste/drag — validates, sanitizes, writes to `assets/` folder |
| `src/commands.ts`     | `openAsText`, `openAsMarkeasy`, `copyContent`, `insertBlock` commands        |
...
2. Add the field to `MarkeasySettings` interface in `extension-host/src/types.ts`
...
5. Add `fontScale: z.number()…` (or equivalent) to `MarkeasySettingsSchema` in both `shared/src/index.ts` and `webview-ui/src/types.ts`
