...
name: Markeasy Project Overview
description: Architecture, tech stack, key files, and current state of the Markeasy VS Code extension
type: project
-------------

# Markeasy — Project Overview

## What the extension does

Markeasy replaces VS Code's default Markdown preview with a live block editor — similar to Notion — that opens automatically when you open any `.md` file. The file on disk is always plain Markdown. The editor is just a visual layer on top of it.
...
When Markeasy opens a file, VS Code runs **two completely separate processes**:
...
- What settings it contributes under `markeasy.*`
...
1. Registers the custom editor provider (so `.md` files open in Markeasy)
...
Defines the `MarkeasySettings` TypeScript interface and two key functions:

- `getSettings()` — reads all `markeasy.*` settings from VS Code and returns them as a typed object
- `getThemeOverrides()` — converts settings into CSS custom property values (e.g. `--markeasy-h1-color: red`) that the webview applies directly

Also contains the colour presets for each code syntax theme (dark, light, GitHub, Monokai).

**Edit this file when you want to:**

- Add a new setting to `MarkeasySettings` (after adding it to `package.json`)
...
Uses CSS custom properties (variables) throughout — most colours come from VS Code's theme via `var(--vscode-*)` variables, and Markeasy's own overrides via `var(--markeasy-*)` variables.
...
Lexical comes with standard nodes (paragraph, heading, list, code, table). Markeasy adds its own for blocks Lexical doesn't support natively:
...
| `.vscode/launch.json`                    | F5 configurations: "Run Markeasy Extension" and "Run Markeasy Extension (with build)" |
